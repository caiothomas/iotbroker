...
var request = require('request'),
    ...
    currentToken = null,
    context = {
        op: 'IoTBroker.Keystone'
    };  

if (config.getConfig().maxQueuedClients) {
    logger.debug('max queued', config.getConfig().maxQueuedClients);
...
function retrieveSubserviceId(req, callback) {
    ...
    function retrieveRequest(innerCb) {
        var options = {
            url: config.getConfig().authentication.options.protocol + '://' + config.getConfig().authentication.options.host + ':' +
            config.getConfig().authentication.options.port + '/v3/projects',
            method: 'GET',
            headers: {
                'X-Auth-Token': currentToken
            },
...
function extractRoles(req, res, next) {
    ...
    if (config.getConfig().authentication.roles) {
        sequence = [
            async.apply(retrieveUser, req)];

        if (config.getConfig().authentication.checkHeaders) {
...
function authenticate(req, res, next) {
    if (authenticating) {
        ...
        logger.debug(context, 'Authenticating PEP Proxy against Keystone: %j', options);
        ...
        request(options, function(error, response, body) {
            ...
            if (error) {
                ...
                logger.error(context, '[VALIDATION-GEN-004] Authentication against Keystone rejected with code %s',
                    response.statusCode);
                waitingRequests.emit('token', new errors.PepProxyAuthenticationRejected(response.statusCode));
            } else {
                logger.error(context, '[VALIDATION-GEN-005] Authentication error trying to authenticate the PEP Proxy: %s',
                    error);
