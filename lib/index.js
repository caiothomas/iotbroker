...
function setAuthenticationModule(moduleName) {
    ...
    if(config.authentication && config.authentication.enable == true)
        logger.info(context, 'Loading Authentication module', config.authentication.module);   
}
...
function initializeProxy(proxyObj, callback){
    ...
    if(config.expiration && config.expiration.enable){
        if(!config.expiration.interval)
            config.expiration.interval = 30000
        setInterval(expiration, config.expiration.interval)            
    }
    ...
    if (config.ssl.active) {
        var sslOptions = {
            ...
            rejectUnauthorized: (config.ssl.rejectUnauthorized) ? true : false
        };
        ...
        proxyObj.proxy.set('portSSL', config.resource.broker.portSSL);

        proxyObj.server = https.createServer(sslOptions, proxyObj.proxy);
        proxyObj.server.listen(proxyObj.proxy.get('portSSL'), proxyObj.proxy.get('host'), function startServer(error) {
            if (error) {
                logger.error(context, 'Error initializing proxy: ' + error.message);

                callback(error);
            } else {
                logger.info(context, 'Proxy listening on port %d with SSL', config.resource.broker.portSSL);
...
function expiration(){
    entityService.removeExpiration(function(){})    
}
