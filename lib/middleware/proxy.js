...
function createOptions(urlList, req, callback){
    ...
    console.log("config.iotAgentSSL", config.iotAgentSSL)
    
    if(config.iotAgentSSL && config.iotAgentSSL.active == true){
        if(config.iotAgentSSL.keyFile)        
            options.key = fs.readFileSync(path.resolve(__dirname, "../../" + config.iotAgentSSL.keyFile));
        if(config.iotAgentSSL.certFile)                
            options.cert = fs.readFileSync(path.resolve(__dirname, "../../"+  config.iotAgentSSL.certFile));          
        options.rejectUnauthorized = (config.iotAgentSSL.rejectUnauthorized) ? true : false;                
        if(config.iotAgentSSL.ca)
            options.ca =  fs.readFileSync(path.resolve(__dirname, "../../" + config.iotAgentSSL.ca));                               
    }
    ...
    if(req.path == "//updateContext")
        pathUrl = "/v1/updateContext";
    else if(req.path == "//queryContext")
        pathUrl = "/v1/queryContext";
    
        logger.debug(context, 'url:', urlList);        
        for(var key in urlList){
            var requestOptions = {
                        uri: urlList[key] +  pathUrl, //req.path,
                        qs: req.query,
                        method: req.method,
                        headers: req.headers                        
            };      
